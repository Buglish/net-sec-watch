...
The analyst workflow usability objective can be marked complete only when an anonymized
...
./scripts/usability-study.py summarize \
  ...
  --output docs/test-results/usability-study.md
```
...
complete. Mark the first analyst workflow completion gate only if the successful
