...
./scripts/benchmark-seven-day-searches.py \
  --insecure \
  --output docs/test-results/search-performance.json
```
...
- **Corpus too small:** load representative retained data or measure the
  production-like environment; do not mark the analyst workflow gate complete.
...
The analyst workflow objective remains open until a committed benchmark artifact records
