...
See [Automatic retention lifecycle verification](opensearch-retention-verification.md)
for the test design and baseline result.

## Development security boundary

This first OpenSearch storage profile follows the official single-node Docker pattern and
...
OpenSearch storage tasks. Do not change `OPENSEARCH_HTTP_BIND` to `0.0.0.0` while this
