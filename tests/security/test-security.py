#!/usr/bin/env python3
"""Static security security contract checks."""
...
def test_roles_and_mappings() -> None:
    ...
    assert_true(set(required) <= set(roles), "missing security roles")
    assert_true(set(required) <= set(mappings), "missing security mappings")
    ...
    assert_true(
        "cluster_all" in roles["net_sec_watch_admin"]["cluster_permissions"],
        "administrator must have cluster_all",
    )
...
def test_classification_rotation_and_supply_chain() -> None:
    ...
    rotation = read("docs/secret-rotation.md")
...
if __name__ == "__main__":
    ...
    for test in tests:
        test()
    print("Security contract is valid.")
